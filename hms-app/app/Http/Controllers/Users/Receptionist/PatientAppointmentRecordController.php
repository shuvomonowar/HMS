...
class PatientAppointmentRecordController extends Controller
{
    ...
    public function index(): View|\Illuminate\Foundation\Application|Factory|Application
    {
        ...
        return view('users.receptionist.patientAppointmentRecords', compact('patientAppointmentRecords'), ['doctors' => $doctors]);
    }
        ...
        try {
            ...
            if ($appointmentDay == $validatedData['appointment_day']) {
                ...
                if ($appointmentRecord) {
                    // dd($validatedData);
                    return redirect()->route('receptionist_user.patient_record.appointment_record.show', $validatedData['patient_id'])->with('add_appointment_success', 'New Appointment record created successfully.');
                } else {
                    // dd($validatedData);
    ...
    public function show(int $patientId): \Illuminate\Foundation\Application|View|Factory|Application
    {
        ...
        return view('users.receptionist.patientAppointmentRecord', compact('patientAppointmentRecords'), ['doctors' => $doctors, 'patientID' => $patientId]);
    }
        ...
        try {
            ...
            if ($appointmentDay == $validatedData['appointment_day']) {
                ...
                $appointmentRecord = $appointmentRecord->update([
                    ...
                    'appointment_serial' => $appointmentCount + 1,
                    'reason' => $validatedData['reason'],
                    'status' => $validatedData['appointment_status'],
                ]);
                ...
                if ($appointmentRecord) {
                    ...
                    // dd($validatedData);
                    return redirect()->route('receptionist_user.patient_record.appointment_record.show', $validatedData['patient_id'])->with('edit_appointment_success', 'Appointment record updated successfully.');
                } else {
                    dd($validatedData);
    ...
    public function search(Request $request, int $patientID): string
    {
        ...
        /*$patientAppointmentRecords = AppointmentRecord::where('patient_id', 'like', '%' . $searchQuery . '%')
            ->orWhere('doctor_name', 'like', '%' . $searchQuery . '%')
            ->orWhere('reason', 'like', '%' . $searchQuery . '%')
            ->orWhereDate('appointment_date', $searchQuery)*/
            /*->orWhereTime('appointment_time', $searchQuery)
        ...
        if ($patientAppointmentRecords->count() >= 1) {
            return view('users.receptionist.patientAppointmentRecordTable', compact('patientAppointmentRecords'))->render();
        } else {
    ...
    /**
     * Show the form for updating the specified patient appointment record status.
     *
     * @param AppointmentRecord $appointmentRecord
     * @return Application|Factory|\Illuminate\Foundation\Application|View
     */
    public function editAppointmentStatus(AppointmentRecord $appointmentRecord): View|\Illuminate\Foundation\Application|Factory|Application
    {
        return view('users.receptionist.updateAppointmentStatusRecord', compact('appointmentRecord'));
    }
     ...
     *
     */
    public function updateAppointmentStatus(AppointmentRecordRequest $request, AppointmentRecord $appointmentRecord): RedirectResponse
    {
        $validatedData = $request->validated();
        ...
        try {
            ...
            $appointmentRecord = $appointmentRecord->update([
                ...
                'appointment_time' => $appointmentTime,
                'appointment_serial' => $validatedData['appointment_serial'],
                'reason' => $validatedData['reason'],
                'status' => $validatedData['appointment_status'],
            ]);
       ...
       /* $appointmentRecord = AppointmentRecord::find($appointmentId)->first();

        $updateResult = $appointmentRecord->update([
            'status' => $request->status, // Assign the status directly
        ]);

        if ($updateResult) {
            return response()->json(['success' => true]);
        } else {
            return response()->json(['success' => false, 'message' => 'Failed to update appointment status'], 500);
        }*/
    }
