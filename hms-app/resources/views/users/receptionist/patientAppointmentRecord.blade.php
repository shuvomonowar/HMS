...
<div class="flex flex-1">
    ...
    <div class="mt-[5rem] ml-[9.2rem] py-1 flex flex-wrap" id="patientAppointmentRecordContainer">
        ...
        <nav class="w-full flex flex-2 gap-7 mb-3 items-center px-4 py-2 border border-blue-100 rounded-lg mr-[3.8rem]">
            ...
            <div>
                <button type="button" id="addAppointmentBtn"
                        class="inline-flex items-center justify-center bg-gray-100 rounded-lg py-1 px-3 transition duration-300 hover:bg-blue-100 hover:ring-1 hover:ring-blue-300">
                    <img src="{{ asset('images/patient/all_appointment.png') }}" alt="View Icon" class="w-6 h-6 mr-2">
                    <span class="text-lg text-blue-400">Add New Appointment</span>
                </button>
            </div>
        </nav>
        ...
        <div class="h-[32.3rem] flex-1 bg-white rounded-lg shadow-md table-container border border-blue-100 mr-[3.9rem]">
            <div class="overflow-x-scroll overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 max-h-full">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    @include('users.receptionist.patientAppointmentRecordTable', ['patientAppointmentRecords' => $patientAppointmentRecords])
                    </tbody>
                </table>
            </div>
        </div>
...
<div id="addAppointmentModal"
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        @include('users.receptionist.addPatientAppointmentRecord')
    </div>
</div>--}}

<!-- Change Appointment Modal -->
<div id="changeAppointmentStatusModal"
     class="fixed inset-0 z-50 hidden overflow-auto bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-md">
        @include('users.receptionist.changeAppointmentStatus')
    </div>
</div>
...
<script>
    ...
    // Search Patient Appointment Record
    $(document).ready(function () {
        $(document).on('keyup', '#searchInput', function (e) {
            e.preventDefault();
            let search_string = $(this).val();
            $.ajax({
                url: "{{ route('receptionist_user.patient_record.appointment_record.search', $patientID) }}",
                method: 'GET',
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const successMessage = '{{ session('add_appointment_success') }}'; // Access flash message
        const errorMessage = '{{ session('add_appointment_error') }}';
    ...
    /*const changeAppointmentStatusBtn = document.getElementById('changeAppointmentStatusBtn');
    const changeAppointmentStatusModal = document.getElementById('changeAppointmentStatusModal');
    const closeStatusModalBtn = document.getElementById('closeStatusModalBtn');

    changeAppointmentStatusBtn.addEventListener('click', () => {
        changeAppointmentStatusModal.classList.remove('hidden');
    });

    closeStatusModalBtn.addEventListener('click', () => {
        changeAppointmentStatusModal.classList.add('hidden');
    });

    function changeAppointmentStatus(appointmentId) {
        const submitStatusBtn = document.getElementById('submit_status_btn');
        const appointmentStatus = document.getElementById('appointment_status');

        submitStatusBtn.addEventListener('click', () => {
            $.ajax({
                url: `/user/receptionist/patient_record/appointment_record/appointment_status/${appointmentId}`,
                type: 'PATCH',
                headers: {
                    'X-CSRF-TOKEN': '{{--{{ csrf_token() }}--}}'
                },
                ...
                success: function (response) {
                    if (response.success) {
                        Command: toastr["success"]("The appointment status is updated successfully.", "Success")
                        ...
                        if (window.confirm('Appointment status updated successfully. Do you want to refresh the page?')) {
                            window.location.reload(); // Reload the page
                        }
                    } else {
                        Command: toastr["error"]("Something went wrong to update the appointment status.", "Error")
                ...
                error: function (error) {
                    ...
                    Command: toastr["error"]("Something went wrong to update this appointment status.", "Error")
    ...
    // Update Appointment Success Message
    document.addEventListener('DOMContentLoaded', function () {
        const successMessage = '{{ session('update_appointment_status') }}'; // Access flash message
