            ...
            <nav class="w-full flex flex-2 gap-7 mb-3 items-center px-4 py-2 border border-blue-100 rounded-lg mr-[3.8rem]">
                ...
                <div>
                    <button type="button" id="addTestBtn"
                        ...
                        <span class="text-lg text-blue-400">Add New Test</span>
                    </button>
                </div>
            </nav>
            ...
            <div
                ...
                <div class="overflow-x-scroll overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 max-h-full">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <caption class="sticky top-0 bg-blue-400 py-0.5 text-lg text-white">Patient Test Record</caption>
                        <tr>
                        ...
                        <!-- Table Body -->
                        <tbody id="patientTestsRecordTable" class="bg-white divide-y divide-gray-200">
                        @include('admin.system.testRecordTable', ['patientTestRecords' => $patientTestRecords])
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        {{--<div class="ml-[82rem] mr-[4rem] bg-blue-400 px-1 py-1.5 items-center rounded-md shadow-2xl">
            <h3 class="text-white font-bold ml-2">Total Cost: <span class="text-red-700 font-bold" id="totalCost"></span></h3>
        </div>--}}
    </div>
</div>
...
<div id="addTestModal"
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        @include('admin.system.addTestRecord')
    </div>
</div>
...
<script>
    ...
    function deletePatientTests(testId) {
        confirmationModal.classList.remove('hidden');

        function confirmDelete() {
            $.ajax({
                url: `/system_admin/patient_record/test_record/${testId}`,
                type: 'DELETE',
                headers: {
                ...
                success: function (response) {
                    if (response.success) {
                        $(`#patientTestsRecordTable tr[data-id='${testId}']`).remove();
    ...
    $(document).ready(function () {
        $(document).on('keyup', '#searchInput', function (e) {
            ...
            $.ajax({
                url: "{{ route('system_admin.test_record.search_tests', $testID) }}",
                method: 'GET',
                data: {search_string: search_string},
                success: function (res) {
                    $('#patientTestsRecordTable').html(res);
                }
            });
        });
    });
    ...
    // Showcase total cost
    $(document).ready(function () {
        let patient_test_id = '{{ $testID }}';
        $.ajax({
            url: `/system_admin/test_records/calculate_total_cost/${patient_test_id}`, // Replace with your backend endpoint
            type: 'GET',
            success: function (response) {
                const totalCostElement = document.getElementById('totalCost');
                if (totalCostElement) {
                    totalCostElement.textContent = response.totalCost + " TK";
                }
            },
            error: function (error) {
                console.error('Error:', error);
